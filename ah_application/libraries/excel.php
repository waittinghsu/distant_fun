		...
		if (!is_array($array)) {
			...
			if (is_array($array)) {
				foreach ($array AS $row) {
					...
					$data .= iconv( "UTF-8", "BIG5//IGNORE", trim($line)) . "\n";
				}
				$data = str_replace("\r", "", $data);
			...
			if (is_null($this->custom_titles)) {
				if (is_array($titles)) {
					foreach ($titles AS $title) {
						$headers[] = iconv( "UTF-8", "BIG5//IGNORE", $title);
					}
				}
				else {
					foreach ($titles AS $title) {
						$headers[] = iconv( "UTF-8", "BIG5//IGNORE", $title->name);
					}
				}
			}
			else {
				$keys = array();
				foreach ($titles AS $title) {
					$keys[] = iconv( "UTF-8", "BIG5//IGNORE", $title->name);
				}
				foreach ($keys AS $key) {
					$headers[] = iconv( "UTF-8", "BIG5//IGNORE", $this->custom_titles[array_search($key, $keys)]);
				}
			}
